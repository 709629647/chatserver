...
void ChatService::addfriend(const TcpConnectionPtr &conn, json &js, Timestamp)
{
    ...
    response["errno"] = 1;
    response["errmsg"] = "add friend err";
    conn->send(response.dump());
}
...
void ChatService::addgroup(const TcpConnectionPtr &conn, json &js, Timestamp)
{
    ...
    json response;
    if (_groupmodel.addGroup(id, role, groupid))
    {
        Group group = _groupmodel.queryGroupdesc(groupid);
